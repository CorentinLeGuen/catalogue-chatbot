...
def chatbot_conversation(messages: List[dict]) -> str:
    ...
    :param messages: The messages history.
    :return: A response.
    """
    if index is None:
        raise RuntimeError("L'application est en cours d'initialisation. Revenez plus tard...")
    last_question = messages[-1]["content"]

    vector = np.array([embed_text(last_question)], dtype=np.float32)
    distance, ids = index.search(vector, k=5)
